...
window.AI = (function () {
  ...
  function buildStoryPrompt(name, kws) {
    return `You write soft, gentle prose. Based on the info below, write an 80-120 word piece about a lost item: "${name}".
Keywords: ${kws.join(', ')}.
Rules:
- English, warm, vivid imagery, not maudlin, no excess metaphor
- Third-person, about the relationship between this object and its owner
- End with a gentle goodbye feeling, but not sad
- Output the paragraph directly — no title, no explanation
- Line breaks allowed, but each block under 3 lines`;
  }

  function localTemplateStory(name, kws) {
    const C = window.COPY;
    const safe = (kws || []).filter(Boolean);
    const a = safe[0] || 'somewhere far';
    const b = safe[1] || 'springtime';
    const c = safe[2] || 'another place';
    const itemName = (name || 'it').replace(/["「」]/g, '');
  ...
  function fallbackSVG(name, keywords) {
    ...
    let p;
    if (/sea|blue|lake|rain|water|ocean/.test(kw)) p = palettes[0];
    else if (/forest|mountain|tree|green|woods/.test(kw)) p = palettes[1];
    else if (/dusk|sunset|evening|autumn|fall/.test(kw)) p = palettes[2];
    else if (/night|moon|star|universe|dark/.test(kw)) p = palettes[3];
  ...
  <text x="50" y="450" font-family="-apple-system,'Songti SC',serif" font-size="22" fill="${inkColor}" opacity="${inkOp}" font-style="italic">${escapeXml((name || '').slice(0, 12))}'s next stop</text>
