...
(() => {
  ...
  function playHeroIntro() {
    ...
    DIU.intro(heroEl, () => {
      // 出台词气泡 + 入口按钮
      bubble.textContent = C.pick(C.diuLines.enter || ['Hello there']);
      bubble.hidden = false;
      SND?.play('chime');
      setTimeout(() => {
  ...
  function refreshLostDeco() {
    ...
    if (total) total.textContent = `${db.lost.length} items`;
    // 兼容旧 IDs
    const tag = $('#todayTag');
    if (tag) tag.textContent = `#${todayCount + 1} / today`;
  ...
  $('#btnVoice').addEventListener('click', () => {
    ...
    if (!SR) { toast('Voice not supported in this browser — typing works too'); return; }
    const r = new SR();
    ...
    r.onerror = () => toast('Did not catch that — try again');
    r.start();
  });
  ...
  $('#searchLightSave')?.addEventListener('click', () => {
    ...
    toast('Parked in Still Out There. Take your time.', 2500);
  ...
  $('#lightDone').addEventListener('click', () => {
    ...
    setTimeout(async () => {
      ...
      // Offer incense after every release (PRD 3.2)
      await offerIncenseOrSkip();
    }, 800);
  });

  // —— Generic: after any "release", offer a gentle incense prompt ——
  async function offerIncenseOrSkip() {
    const item = lostState.currentId ? findLost(lostState.currentId) : null;
    const itemName = item?.name || lostState.name || 'it';
    const yes = await modal(
      `<h3>Light a stick for "${escapeHTML(itemName)}"?</h3>
       <p>A 30-second little ritual — a gentle goodbye.<br>Skipping is fine too.</p>`,
      [
        { label: 'No thanks, to map' },
        { label: '🕯 Light one', primary: true },
      ]
    );
    if (yes === 1 && item) {
  ...
  $('#inPhoto')?.addEventListener('change', e => {
    ...
    if (f.size > 8 * 1024 * 1024) {
      toast('Image too big — under 8MB please');
      e.target.value = '';
      return;
    }
  ...
  $('#cardSave').addEventListener('click', async () => {
    SND?.play('drop'); HAP?.light();
    const ok = await FC.save(`misplaced-${lostState.name || ''}-${Date.now()}.png`);
    if (ok) { SND?.play('chime'); HAP?.confirm(); toast('Saved to your device.', 2000); }
  });
  ...
  $('#cardShare').addEventListener('click', async () => {
    ...
    if (r.ok) { SND?.play('chime'); HAP?.confirm(); }
      ...
      toast('Native sharing not supported — try saving the image instead', 3000);
    } else {
      toast('Share failed — try saving locally first', 2500);
    }
  });
  ...
  $('#searchToRelease').addEventListener('click', async () => {
    HAP?.light();
    await modal(
      `<h3>Done waiting?</h3><p style="color:var(--ink-soft);font-size:14px;line-height:1.6">I will move it to the Star Map as Released. You can also walk with it through the healing flow.</p>`,
      [
        { label: 'Cancel' },
        { label: 'Just release', onClick: async () => {
          if (lostState.currentId) updateLost(lostState.currentId, { status: 'released' });
          SND?.play('whoosh'); HAP?.success();
          await offerIncenseOrSkip();
        }},
        { label: 'Walk with it', primary: true, onClick: () => {
          $('#bridgeLine').textContent = C.pick(C.heavyBridge);
          DIU.render('#diuHeavy', 'sad', C.pick(C.diuLines.heavy), 'sad');
          showLostView('path-heavy-bridge');
        }}
  ...
  function addKw(text) {
    ...
    if (lostState.storyKws.length >= 6) {
      toast('Six is plenty — more makes it harder to write.', 2000);
      return;
    }
  ...
  $('#noteAdd').addEventListener('click', () => {
    ...
    if (!txt) { toast('Write a few words first'); HAP?.error(); return; }
  ...
  function renderStillhere() {
    ...
    if (!items.length) {
      list.innerHTML = `<div class="empty-state"><div class="empty-ghost">◐</div><p>Empty here.<br>That is a good thing.</p></div>`;
      return;
    }

    list.innerHTML = items.map(it => {
      const tagIcon = it.kind === 'note' ? '🗒' : '🔍';
      const tagText = it.kind === 'note' ? 'Reminder' : (it.noLocation ? 'No clue' : 'Searching');
      const remind = it.remindAt ? ` · reminds ${fmtTime(it.remindAt)}` : '';
      return `
        <div class="sh-item ${it.kind} ${it.done ? 'done' : ''}" data-id="${it.id}" data-kind="${it.kind}">
          ...
          <button class="sh-check" data-toggle aria-label="Done">✓</button>
          <button class="sh-del" data-del aria-label="Delete">×</button>
        </div>
      `;
    }).join('');
  ...
  function renderMuseum() {
    ...
    if (empathy) {
      empathy.textContent = C.starmapTagline || 'Every misplaced thing becomes a star.';
  ...
  function renderTimeline(items) {
    ...
    if (!list) return;
    if (!items.length) {
      list.innerHTML = `<div class="empty-state"><div class="empty-ghost">≡</div><p>No closed records yet.</p></div>`;
      return;
    }
    // 按月分组
    const grouped = {};
    ...
    list.innerHTML = keys.map(k => {
      const monthLabel = new Date(k.replace('.', '-') + '-01').toLocaleDateString('en-US', {month: 'long', year: 'numeric'});
      const items = grouped[k];
      return `
        <div class="tl-month">${monthLabel}</div>
        ${items.map(r => {
          const status = ({
            released: 'Released', healed: 'Sealed', incensed: 'Incensed', found: 'Found', searching: 'Searching'
          })[r.status] || r.status;
          const d = new Date(r.createdAt);
  ...
  async function openMuseumItem(id) {
    ...
    const statusLabel = {
      searching: 'Searching', found: 'Found', released: 'Released',
      healed: 'Sealed', incensed: 'Incensed'
    }[r.status];

    let body = `
      <h3>${escapeHTML(r.name)}</h3>
      <p style="color:var(--ink-soft);font-size:13px;margin:0 0 16px;">
        ${fmtTime(r.createdAt)} · ${statusLabel}${r.incenseSticks ? ` · ${r.incenseSticks} sticks` : ''}
      </p>
    `;
    if (r.place) body += `<p style="font-size:14px;margin:6px 0;"><b style="color:var(--ink-soft);">Last seen: </b>${escapeHTML(r.place)}</p>`;
    if (r.desc) body += `<p style="font-size:14px;margin:6px 0;"><b style="color:var(--ink-soft);">Notes: </b>${escapeHTML(r.desc)}</p>`;
    if (r.contact) body += `<p style="font-size:14px;margin:6px 0;"><b style="color:var(--ink-soft);">Contact: </b>${escapeHTML(r.contact)}</p>`;
    ...
    if (r.status === 'searching') {
      actions.push({ label: 'Found it', primary: true, onClick: () => {
        updateLost(id, { status: 'found' });
        SND?.play('found'); HAP?.success();
        toast(C.pick(C.foundLines), 3000);
        renderMuseum();
      }});
      actions.push({ label: 'Stop looking', onClick: () => {
        updateLost(id, { status: 'released' });
        SND?.play('whoosh'); HAP?.light();
        toast(C.pick(C.noBlame), 3000);
        renderMuseum();
    ...
    if (r.status !== 'incensed') {
      actions.push({ label: '🕯 Incense', onClick: () => {
        IN.open(r, ({ sticks, at }) => {
    ...
    actions.push({ label: 'Delete', danger: true, onClick: async () => {
      const yes = await modal('<h3>Remove from map?</h3><p style="color:var(--ink-soft);font-size:14px;">This record will disappear. No undo.</p>',
        [{ label: 'Cancel' }, { label: 'Delete', primary: true, danger: true }]);
      if (yes === 1) {
  ...
  function renderMe() {
    ...
    if (recent.length === 0) {
      $('#blameIndex').textContent = 'No data yet. Log a few things and check back.';
    } else {
      ...
      let line = '';
      if (ratio > 0.6) line = `This month, mostly «💚 no big deal» entries. Doing well 👍`;
      else if (heavy > 2) line = `${heavy} heavier ones this month. Take your time — it is okay.`;
      else line = `${recent.length} records this month. All of them are with me now — you do not need to carry them.`;
  ...
  $('#toggleMusic')?.addEventListener('click', () => {
    ...
    if (on) {
      ...
      const t = db.settings.musicTheme || 'warm';
      window.MUSIC?.start(v, t);
      toast('Music on · quietly with you', 2200);
    } else {
      window.MUSIC?.stop();
      toast('Music off', 1500);
    }
  });
  ...
  $('#permBtn').addEventListener('click', async () => {
    ...
    if (r === 'granted') {
      ...
      toast('Enabled. I will nudge you gently at the right moments.', 2500);
      SND?.play('chime');
    } else { toast('No worries — works fine without.'); }
  });
  $('#testNotify').addEventListener('click', () => {
    if (Notification.permission !== 'granted') { toast('Enable permissions first'); return; }
    new Notification('Misplaced', { body: 'Just a test. Nothing actually lost.', icon: 'icons/icon-192.svg' });
  });
  ...
  $('#wipeBtn').addEventListener('click', async () => {
    HAP?.error();
    const yes = await modal(
      '<h3>Wipe everything?</h3><p style="color:var(--ink-soft);font-size:14px;">All your records and notes will be erased. No undo.</p>',
      [{ label: 'Nevermind' }, { label: 'Wipe', primary: true, danger: true }]
    );
    if (yes === 1) {
      db.lost = []; db.notes = []; saveDB();
      SND?.play('whoosh'); HAP?.success();
      toast('All clear. Fresh start.', 2500);
      renderMe();
    }
  });
  ...
  function triggerReminder(type, id) {
    if (type === 'note') {
      ...
      if (Notification.permission === 'granted') {
        new Notification('Misplaced · Reminder', { body: msg, icon: 'icons/icon-192.svg' });
      } else {
        toast('Reminder: ' + msg, 4000);
        SND?.play('drop');
      }
    } else if (type === 'search') {
      const r = findLost(id);
      if (!r || r.status !== 'searching') return;
      const msg = '«' + r.name + '» from 3 days ago — still looking?';
      if (Notification.permission === 'granted') {
        new Notification('Misplaced · Nudge', { body: msg, icon: 'icons/icon-192.svg' });
      } else {
        toast(msg, 4000);
      }
    }
  ...
  function openAbout() {
    ...
    $('#aboutContent').innerHTML = `
      ...
      <div class="about-meta">About · Zeigarnik Effect</div>
      ${html}
