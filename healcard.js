...
window.HEALCARD = (function () {
  ...
  async function render(data) {
    ...
    const name = (data.name || 'it').slice(0, 18);
    ...
    ctx.fillText('now settled in your star map', W / 2, y);
    y += 50;
    ...
    if (data.epitaph && data.epitaph.trim()) {
      ...
      ctx.fillText('It left gently,', W / 2, y); y += 28;
      ctx.fillText('but it had a place here.', W / 2, y); y += 40;
    }
    ...
    ctx.fillText('— sealed by Misplaced', 80, footY + 12);
    ctx.fillStyle = 'rgba(244,240,232,0.42)';
    ctx.font = "400 12px 'Songti SC', 'PingFang SC', serif";
    ctx.fillText('Every misplaced thing becomes a star in your map.', 80, footY + 42);
    ctx.font = "500 11px 'SF Mono', monospace";
    ctx.fillStyle = 'rgba(244,240,232,0.40)';
    ctx.fillText('#Misplaced  #Memorial  #StarMap', 80, footY + 70);
  }
  ...
  async function save(filename = 'misplaced-memorial.png') {
  ...
  async function share(name) {
    ...
    const file = new File([blob], `misplaced-memorial-${Date.now()}.png`, { type: 'image/png' });
    try {
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: `In memory of "${name || ''}"`,
          text: `It is now in my star map. — Misplaced`,
          files: [file],
        });
        return { ok: true };
      }
      if (navigator.share) {
        await navigator.share({
          title: 'Memorial',
          text: `"${name || ''}" is now in my star map.`,
        });
