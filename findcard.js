...
window.FINDCARD = (function () {
  ...
  async function render(data) {
    ...
    ctx.fillText('✦   L O S T   &   F O U N D   ✦', W / 2, 100);
    ...
    ctx.textAlign = 'center';
    ctx.fillText('I lost', W / 2, labelY);

    // 巨大物品名
    let y = labelY + 56;
    ctx.fillStyle = '#1f1d1a';
    ctx.font = 'bold italic 52px "Songti SC", "Noto Serif SC", Georgia, serif';
    const name = (data.name || 'a small unnamed thing').slice(0, 20);
    ...
    ctx.fillText('If a kind soul sees this,', W / 2, y);
    y += 32;
    ctx.fillText('could you bring it back?', W / 2, y);
    y += 26;

    // 落款：— 圆滚滚的丢丢说
    ctx.font = '14px "Songti SC", "PingFang SC", serif';
    ctx.fillStyle = '#ff8a9c';
    ctx.fillText('— says round little Lossy', W / 2, y);
    y += 40;
    ...
    drawField('PLACE', data.place);
    drawField('DETAILS', data.desc);
    drawField('CONTACT', data.contact);
    ...
    ctx.fillText('— made by Misplaced', 80, footY + 12);
    ctx.fillStyle = '#a89e8a';
    ctx.font = '400 13px "Songti SC", "PingFang SC", serif';
    ctx.fillText('Share it · more eyes, more hope', 80, footY + 42);
    // 极小 hashtag
    ctx.font = '500 11px "SF Mono", monospace';
    ctx.fillStyle = '#b8a896';
    ctx.fillText('#Misplaced  #LostAndFound', 80, footY + 70);
  }
  ...
  async function drawPolaroidWithMascot(c, photoImg, cx, cy, photoW, photoH) {
    ...
    c.fillText('· before it was lost ·', 0, -frameH / 2 + padTop + photoH + padBottom / 2);
    ...
    // 3) 小说话框：从丢丢指向照片
    drawCanvasBubble(c, mx + mSize * 0.5, my - 18, 'Look, this is it');
  }
  ...
  async function save(filename = 'misplaced-lost.png') {
  ...
  async function share(name) {
    ...
    const file = new File([blob], `misplaced-lost-${Date.now()}.png`, { type: 'image/png' });
    try {
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: `Lost: ${name || ''}`,
          text: `I lost "${name || ''}". If a kind soul sees this, could you bring it back?`,
          files: [file],
        });
        return { ok: true };
      }
